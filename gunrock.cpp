...
void* consumer(void *arg){
  while(true){
    ...
    while(client_buffer.empty()){
      dthread_cond_wait(&filled, &mutex);
    }
    MySocket *tmp = client_buffer.front();
    client_buffer.pop_front();
    dthread_cond_signal(&empty);
...
int main(int argc, char *argv[]) {
  ...
  for(int i = 0; i < THREAD_POOL_SIZE; i++){
    dthread_create(&thread_pool[i], NULL, consumer, NULL);
  }
  ...
  while(true) {
    ...
    dthread_mutex_lock(&mutex);
    while(client_buffer.size() == BUFFER_SIZE){
      dthread_cond_wait(&empty, &mutex);
    }
